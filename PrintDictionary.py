...
class PrintDictionary(object):
    ...
    def formatted_dict (self, some_dict_col, format_dict = None):
        ...
        for key in color:
            if isinstance(color[key], str):
                color[key] = [ color[key] ]
            else:
                #No naughty spaces in our color lists please.
                color[key] = [str(x).strip() for x in color[key]]
        #We now modify the colors dictionary again and we make sure that each of the keys
        #has at least some value associated with it even if it is just the reset values.
        reset = [self.TOTAL_RESET]
        ...
        for index, ourDict in enumerate(some_dict_col):
            some_dict_temp = {'_COUNTER' : counter}
            for key in ourDict:
...
if __name__ == '__main__':
    ...
    print(format_dict['in_between'])
    y = x.print_dict( temp,   format_dict)